...
app.get("/api/projects/:id/logs", async (req, res) => {
  ...
  try {
    const [logs] = await connection.query(
      `SELECT logs.*, u.username AS user_name
       FROM activity_logs logs
       JOIN users u ON logs.user_id = u.user_id
    ...
    console.error("로그 조회 중 오류 발생:", err);
...
app.post("/api/tasks/:taskId/comments", async (req, res) => {
  ...
  try {
    ...
    // 활동 로그 기록
    await logActivity(connection, user_id, projectId, taskId, "댓글 작성", {
...
app.delete("/api/comments/:commentId", async (req, res) => {
  ...
  try {
    ...
    const [commentRows] = await connection.query(
      "SELECT user_id, content, task_id FROM comments WHERE comment_id = ?",
      [commentId]
    );
    ...
    if (!projectId) {
      await connection.rollback();
      return res.status(500).json({ error: "프로젝트 ID를 찾을 수 없습니다." });
    }
    ...
    if (deleteResult.affectedRows === 0) {
      await connection.rollback();
      return res.status(404).json({ error: "삭제할 댓글을 찾을 수 없습니다 (삭제 단계)." });
    }

    // 활동 로그 기록
    await logActivity(connection, userId, projectId, comment.task_id, "댓글 삭제", {
...
app.put("/api/projects/:projectId/members/:memberId", async (req, res) => {
  ...
  try {
    ...
    const oldRole = existingRoleRows[0]?.role_in_project ?? null;
    ...
    const [targetUser] = await connection.query(
      "SELECT username FROM users WHERE user_id = ?", [memberId]
    );
